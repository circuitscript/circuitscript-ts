...
export class BaseVisitor extends CircuitScriptParserVisitor<ComplexType | AnyReference | any> {
    ...
    enableCachedImports = false;

    onImportFile = (visitor: BaseVisitor, filePath:string, fileData: string, onErrorHandler: OnErrorHandler): ImportFileResult => {
        ...
        try {
            if (pathExists && filePathUsed) {
                ...
                if (this.enableCachedImports) {
                    try {
                        ...
                        if (cachedIR !== null) {
                            ...
                            const importContext = executionStack[executionStack.length - 1];

                            const parseAndVisit = (miniScript: string): ImportFileResult => {
                                return this.onImportFile(this, filePathUsed, miniScript, this.onErrorHandler);
                            };
                ...
                if (!loadedFromCache) {
                    ...
                    if (this.enableCachedImports && !hasError && !hasParseError && contentHash !== null) {
                        try {
