...
import { SerializedExpression } from './cache/types.js';
import { Frame } from './objects/Frame.js';
import { Wire } from './objects/Wire.js';
import { NetClass } from './objects/NetClass.js';
import { ERC_RuleSeverity } from './rules-check/severity-defaults.js';
...
export class BaseVisitor extends CircuitScriptParserVisitor<ComplexType | AnyReference | any> {
    ...
    wireCtxLinks = new Map<Wire, ParserRuleContext>;
    ...
    constructor(silent = false, 
        ...
        for (const [rule, severity] of Object.entries(ERC_RuleSeverity)) {
            defaultRules[rule] = severity;
        }
        ...
        this.startingContext.scope.netMap.componentPinNetPairResolver =
            this.createComponentPinNetPairResolver(this.executionStack);

        this.startingContext.resolveScenario = 
            this.createScenarioResolver(this.executionStack);
    ...
    createNetResolver(executionStack: ExecutionContext[]):
        ...
        return (netName: string, netNamespace: string): { found: boolean, net?: Net } => {
    ...
    createComponentPinNetResolver(executionStack: ExecutionContext[]):
        ...
        // The component pin net pair resolver is used internally because
        // the resolving is the same.
        const resolver = this.createComponentPinNetPairResolver(executionStack);
        return (component: ClassComponent, pin: PinId): Net | null => {
            const result = resolver(component, pin);
            if (result !== undefined) {
                return result[2];
            }
    ...
    // Create resolve function to find/resolve component pin net pair.
    createComponentPinNetPairResolver(executionStack: ExecutionContext[]):
        (component: ClassComponent, pin: PinId) => ComponentPinNetPair | undefined {

        return (component: ClassComponent, pin: PinId): ComponentPinNetPair | undefined => {
            const reversed = [...executionStack].reverse();

            for (let i = 0; i < reversed.length; i++) {
                const context = reversed[i];
                // The `resolve` param is set to false, so that only items within
                // the scope's netMap are searched and does not go above to 
                // parent contexts. Otherwise this function is calling itself.
                const pair = context.scope.netMap.findNet(component, pin, false);
                if (pair !== undefined) {
                    return pair;
                }
            }
    ...
    // Resolve scenarios from upper contexts.
    createScenarioResolver(executionStack: ExecutionContext[]): () => Scenario | null {
        return () => {
            const reversed = [...executionStack].reverse();

            for (let i = 0; i < reversed.length; i++) {
                const context = reversed[i];
                if (context.scope.scenario !== null) {
                    return context.scope.scenario;
                }
            }
    ...
    protected resolveTrailersForReference(reference: AnyReference,
        passedNetNamespace: string | null, ctx: Callable_exprContext | Double_dot_property_set_exprContext): AnyReference {
        ...
        if (trailers.length === 0) {
            ...
            if (leftSideReference.rootValue instanceof ClassComponent) {
                ...
                leftSideReference.rootValue.parameters.set(trailers[0], rhsValue);
                    ...
                    || leftSideReference.rootValue instanceof NetClass){
    ...
    protected handleImportFile(name: string, 
        ...
        // Extract the original import statement and store location
        let importStatement!: SerializedExpression;
        if (ctx !== null) {
    ...
    protected setInstanceParam(object: ClassComponent, trailers: string[], value: any): void {
        if (trailers.length === 1){
            ...
            // Only apply these params to the unit.
            const unitModifiers = [
                ParamKeys.angle,
                ParamKeys.flip,
                ParamKeys.flipX,
    ...
    protected enterNewChildContext(executionStack: ExecutionContext[],
        ...
        newExecutor.resolveScenario = this.createScenarioResolver(executionStack);
