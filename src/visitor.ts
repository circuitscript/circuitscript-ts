...
import { SymbolDrawing } from './draw_symbols';
...
export class MainVisitor extends ParseTreeVisitor<any> {
    ...
    visitCreate_graphic_expr(ctx: Create_graphic_exprContext) {
        ...
        const symbolDrawing = new SymbolDrawing();

        ctx.sub_expr_list().forEach(item => {
            const [commandName, parameters] = this.visit(item);

            const tmpParams = parameters.map(([, , value]) => {
                return value;
            }) as any[];

            if (commandName === 'rect') {
                symbolDrawing.addRect(...tmpParams);

            } else if (commandName === 'pin') {
                symbolDrawing.addPin(...tmpParams);

            } else if (commandName === 'label') {
                symbolDrawing.addLabel(...tmpParams);
            }
        });
