...
export class MainVisitor extends CircuitScriptVisitor<any> {
    ...
    constructor(silent = false, onErrorHandler: OnErrorCallback | null = null) {
        super();
    ...
    //         const tmpException = new VisitorExecutionException(ctx, err);
    //         this.onErrorCallbackHandler &&
    //             this.onErrorCallbackHandler(ctx.start.line, ctx.start.column, "", tmpException);
    //         throw tmpException;
    //     } else {
    //         throw err;
    //     }
    ...
    visitAtom_expr = (ctx: Atom_exprContext): ReferenceType => {
        ...
        if (ctx.parentCtx instanceof ExpressionContext && !currentReference.found) {
    ...
    private importLib(visitor: MainVisitor, directory: string, 
        filename: string): 
        { hasError: boolean, hasParseError: boolean, pathExists: boolean } {
